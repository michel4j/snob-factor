...
void peek_data(SnobContext *ctx) {
  ...
  for (int i = 0; i < ctx->state.vset->length; i++) {
    if ((ctx->state.vset->length > 10) && (i > 5) &&
        (i < ctx->state.vset->length - 5))
      continue;
  ...
  for (int n = 0; n < ctx->state.sample->num_cases; n++) {
    if ((ctx->state.sample->num_cases > 20) && (n > 5) &&
        (n < ctx->state.sample->num_cases - 5))
      continue;
    field =
        ctx->state.sample->records + n * ctx->state.sample->record_length + 1;
    caseid = *(int *)(field);
...
int create_vset(SnobContext *ctx, const char *name, int num_vars) {
  ...
  if (found >= 0) {
    ...
    vset_var_list = (VSetVar *)alloc_blocks(ctx, 3, num_vars * sizeof(VSetVar));
    if (!vset_var_list) {
      return error_value(ctx, "Cannot allocate memory for variables blocks",
                         -3);
    }
    ctx->state.vset->variables = vset_var_list;
...
int create_sample(SnobContext *ctx, char *name, int size, int *units,
  ...
  if (find_sample(ctx, name, 0) >= 0) {
    ...
    do {
      ...
      strcpy(ctx->state.sample->vset_name,
             ctx->state.vset->name); /*	Set variable-set name in sample  */
      smpl_var_list = (SampleVar *)alloc_blocks(
          ctx, 0, ctx->state.vset->length * sizeof(SampleVar));
      if (!smpl_var_list) {
      ...
      for (int i = 0; i < ctx->state.vset->length; i++) {
        ...
        saux = (char *)alloc_blocks(
            ctx, 0, vtype->smpl_aux_size); /*	Make the saux block  */
        if (!saux) {
      ...
      ctx->state.sample->num_active = 0;
      ctx->state.sample->records = field =
          (char *)alloc_blocks(ctx, 0, size * record_length);
      if (!field) {
        log_msg(ctx, 2, "No space for data");
        out = -8;
        break;
...
int add_record(SnobContext *ctx, int index, char *bytes) {
  ...
  char *field = (char *)ctx->state.sample->records +
                ctx->state.sample->num_added * ctx->state.sample->record_length;
  if (caseid < 0) {
    caseid = -caseid;
    *field = 0;
  } else {
