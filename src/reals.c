...
void show(SnobContext *ctx, Class *cls, int iv) {
  ...
  printf("V%3d  Cnt%6.1f  %s\n", iv + 1, exp_var->cnt,
         (cls_var->infac) ? " In" : "Out");
  if (cls->num_sons > 1) {
    printf(" N: Cost%8.1f  Mu%8.3f+-%8.3f  SD%8.3f+-%8.3f\n", exp_var->npcost,
           cls_var->nmu, sqrt(cls_var->nmusprd), exp(cls_var->nsdl),
           exp(cls_var->nsdl) * sqrt(cls_var->nsdlsprd));
  }
  printf(" S: Cost%8.1f  Mu%8.3f  SD%8.3f\n", exp_var->spcost + exp_var->stcost,
         cls_var->smu, exp(cls_var->ssdl));
  printf(" F: Cost%8.1f  Mu%8.3f  SD%8.3f  Ld%8.3f\n",
         exp_var->fpcost + exp_var->ftcost, cls_var->fmu, exp(cls_var->fsdl),
         cls_var->ld);
}
...
void details(SnobContext *ctx, Class *cls, int iv, MemBuffer *buffer) {
  ...
  if (cls->use == Fac) {
    print_buffer(ctx, buffer,
                 "\"mean\": %0.5f, \"stdev\": %0.5f, \"loading\": %0.3f",
                 cls_var->fmu, exp(cls_var->fsdl), cls_var->ld);
  } else {
    print_buffer(ctx, buffer, "\"mean\": %0.5f, \"stdev\": %0.5f", cls_var->smu,
                 exp(cls_var->ssdl));
  }
  print_buffer(ctx, buffer, "}");
}
