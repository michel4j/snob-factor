...
void select_sample(SnobContext *ctx, char *name) {
    ...
    if (smpl_id >= 0) {
        log_msg(ctx, 1, "Selecting sample [%d] %s", smpl_id, ctx->samples[smpl_id]->name);
        ...
        if (k >= 0) {
            ...
            log_msg(ctx, 0, "Preparing Initial Population for sample [%d] %s", smpl_id, ctx->samples[smpl_id]->name);
...
void show_population(SnobContext *ctx) {
    ...
    if (popln->sample_size) {
        log_msg(ctx, 2, "P%1d  %4d classes, %4d leaves,  Pcost%8.1f  Tcost%10.1f,  Cost%10.1f", popln->id + 1,
                popln->num_classes, popln->num_leaves, root->best_par_cost, root->best_case_cost, root->best_cost);
    } else {
        log_msg(ctx, 2, "P%1d  %4d classes, %4d leaves,  Pcost%8.1f", popln->id + 1, popln->num_classes,
                popln->num_leaves, root->best_par_cost);
    }
    log_msg(ctx, 2, "Sample %2d %s", (sample) ? sample->id + 1 : 0, (sample) ? sample->name : "NULL");
    log_msg(ctx, 2,
            "--------------------------------------------------------------------"
            "------------");
}
...
Result classify(SnobContext *ctx, const int max_cycles, const int do_steps, const int move_steps, const double tol) {
    ...
    if (cycle >= max_cycles) {
        log_msg(ctx, 1, "WARNING: Classification did not converge after %d cycles", max_cycles);
    } else if (ctx->stop) {
        log_msg(ctx, 1, "WARNING: Classification interrupted after %d cycles", cycle);
    } else {
