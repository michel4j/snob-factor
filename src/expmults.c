...
typedef struct Basicst { /* Basic parameter info about var in class.
  ...
  double napsprd; /* basically, the squared sprd among the ap[] params of
                     children  */
  double sapsprd;
  double fapsprd;
  double bpsprd;
...
typedef struct Statsst { /* Stuff accumulated to revise Basic  */
  ...
  double ff, parkb1p, parkb2p,
      conff; /* Useful quants calced in cost_var, used in deriv_var */
  double oldftcost;
  double adj;
  double apd2, bpd2;
...
sample.
    */
int read_attr_aux(SnobContext *ctx, void *vaux) {
  int i, states;

  /*	Read in auxiliary info into vaux, return 0 if OK else 1  */
  i = read_int(ctx, &states, 1);
  if (i < 0) {
    return (1);
  } else {
    return set_attr_aux(ctx, vaux, states);
  }
}
int set_attr_aux(SnobContext *ctx, void *vaux, int states) {
...
int read_smpl_aux(SnobContext *ctx, void *sax) {
  return (0);
} /*	Multistate has no auxilliary info re sample  */
int set_smpl_aux(SnobContext *ctx, void *sax, int unit, double prec) {
  return (0);
}
...
void set_probs(SnobContext *ctx, int iv, Prob *probs, Class *cls) {
  double sum, tt, lsum;
  int k, ig;
...
void deriv_var(SnobContext *ctx, int iv, int fac, Class *cls) {
  ...
  scnt[saux->xn] +=
      case_weight; /* For non-fac, I just accumulate counts in scnt[]  */
  exp_var->stcost +=
      case_weight * scst[saux->xn]; /* Accum. weighted item cost  */
  exp_var->ftcost +=
      case_weight * exp_var->parkftcost; /* Accum. weighted item cost  */
...
/*	------------------------  details  -----------------------   */
/*	prints the exponential params in 'ap' converted to probs   */
void write_probs(SnobContext *ctx, double *ap, int states, double *pr,
                 MemBuffer *buffer) {
  int k;
  double max, sum;
  ...
  for (k = 0; k < states; k++) {
    ...
    print_buffer(ctx, buffer, "%0.3f", pr[k] * sum);
  }
  return;
}
...
void details(SnobContext *ctx, Class *cls, int iv, MemBuffer *buffer) {
  ...
  if (cls->num_sons > 1) {
    print_buffer(ctx, buffer, "\"frequency\": [");
    write_probs(ctx, nap, states, pr, buffer);
    print_buffer(ctx, buffer, "]");
  } else if (cls->use == Fac) {
    print_buffer(ctx, buffer, "\"frequency\": [");
    for (k = 0; k < states; k++) {
      ...
      print_buffer(ctx, buffer, "%0.3f", frate[k]);
    }
    print_buffer(ctx, buffer, "], ");
    print_buffer(ctx, buffer, "\"influence\": [");
    for (k = 0; k < states; k++) {
      if (k > 0) {
        print_buffer(ctx, buffer, ", ");
      }
    ...
    print_buffer(ctx, buffer, "\"frequency\": [");
    write_probs(ctx, sap, states, pr, buffer);
    print_buffer(ctx, buffer, "]");
  }
  print_buffer(ctx, buffer, "}");
}
