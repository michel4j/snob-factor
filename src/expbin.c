...
typedef struct Basicst { /* Basic parameter info about var in class.
  ...
  double
      napsprd; /* basically, the squared sprd among the ap params of children */
  double sapsprd;
  double fapsprd;
  double bpsprd;
...
static void reduce_stats(SnobContext *ctx, int iv, Class *dest, Class *src) {
  ...
  d_exp->vsq += s_exp->vsq;
  d_exp->cnt1 += s_exp->cnt1;
  d_exp->fapd1 += s_exp->fapd1;
  d_exp->fbpd1 += s_exp->fbpd1;
  d_exp->apd2 += s_exp->apd2;
  d_exp->bpd2 += s_exp->bpd2;
  d_exp->tvsprd += s_exp->tvsprd;
}
...
void details(SnobContext *ctx, Class *cls, int iv, MemBuffer *buffer) {
  ...
  if (cls->use == Fac) {
    print_buffer(ctx, buffer, "\"frequency\": %0.3f, ", cls_var->fap);
    print_buffer(ctx, buffer, "\"influence\": %0.3f", cls_var->fbp);
  } else {
    print_buffer(ctx, buffer, "\"frequency\": %0.3f", cls_var->sap);
  }
