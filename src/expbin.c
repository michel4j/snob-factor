...
adjloop:
    ...
    pcost = del * del;
    pcost = 0.5 * (pcost + apsprd / nson) / cls_var->dapsprd + (SNOB_HALF_LOG_2PI + 0.5 * log(cls_var->dapsprd));
    //      Add hlog Fisher, lattice
    pcost += 0.5 * log(0.5 * nson + nints) + 0.5 * log((double)nson) - 1.5 * log(apsprd) + 2.0 * SNOB_LATTICE;
