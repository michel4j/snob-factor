...
int make_population(SnobContext *ctx, int fill) {
  ...
  if ((!ctx->state.sample) && fill) {
    return error_value(ctx, "Makepop cannot fill because no sample defined",
                       -1);
  }
  ...
  popln->variables =
      (PopVar *)alloc_blocks(ctx, 1, ctx->state.vset->length * sizeof(PopVar));
  ...
  popln->classes =
      (Class **)alloc_blocks(ctx, 1, MAX_CLASSES * sizeof(Class *));
  if (!popln->classes) {
    return error_value(ctx, "No space for another population", -1);
  }
...
int copy_population(SnobContext *ctx, int p1, int fill, char *newname) {
  ...
  if (ctx->state.sample &&
      (!strcmp(ctx->state.sample->vset_name, ctx->state.vset->name))) {
...
sampfound:
  ...
  if (i == p1) {
    log_msg(ctx, 1, "From copypop: attempt to copy model%3d into itself",
            p1 + 1);
    indx = -102;
    goto finish;
  }
...
void print_tree(SnobContext *ctx) {
  ...
  printf("Popln%3d on sample%3d,%4d classes,%6d things", popln->id + 1,
         ctx->state.sample->id + 1, popln->num_classes,
...
int get_best_pop(SnobContext *ctx) {
  ...
  if ((ctx->state.popln) && (ctx->state.sample)) {
    ...
    if (i < 0) {
      ...
      ctx->state.sample->best_cost =
          ctx->state.popln->classes[ctx->state.popln->root]->best_cost;
      /*	Set goodtime as current pop age  */
      ctx->state.sample->best_time =
          ctx->state.popln->classes[ctx->state.popln->root]->age;
    }
  }
  return (i);
}
...
int save_population(SnobContext *ctx, int p1, int fill, char *newname) {
  ...
  fclose(file_ptr);
  log_msg(ctx, 1, "\nModel %s  Cost %10.2f  saved in file %s", oldname,
