...
gotit:
    ...
    if (fill) {
        ...
        CurCtx.popln->sample_size = num_cases;
    }
...
int copy_population(int p1, int fill, char *newname) {
    ...
    EVinst *stats, *fevi;
    double nomcnt;
    int indx, sindx, kk, jdad, nch, n, i, iv, hiser, num_cases;
...
int load_population(char *nam) {
    ...
    Context oldctx;
    int i, j, k, indx, fncl, fnc, nch, iv, num_cases;
    Class *cls;
    CVinst *cvi;
    EVinst *stats;
    FILE *fl;
...
void correlpops(int xid) {
    ...
    if (xpop->sample_size != CurCtx.sample->num_cases) {
