...
void print1class(Class *clp, int full) {
    ...
    printf(" +-%5.3f", (clp->vav));
    printf(" Avv%6.3f", clp->avg_factor_score);
    printf("%c", (clp->use == Fac) ? ' ' : ')');
    if (clp->type == Dad) {
    ...
    printf("Tcosts  S:%9.2f  F:%9.2f  D:%9.2f  B:%9.2f\n", clp->cstcost, clp->cftcost - cls->cfvcost, clp->cntcost, clp->best_par_cost);
    printf("Vcost     ---------  F:%9.2f\n", clp->cfvcost);
...
void scorevarall(Class *ccl) {
    ...
    if ((cls->age < MinFacAge) || (cls->use == Tiny)) {
        cvv = cls->avg_factor_score = cls->sum_score_sq = 0.0;
        icvv = 0;
        goto done;
    }
...
void adjustclass(Class *ccl, int dod) {
    ...
    /*	Get average score   */
    cls->avg_factor_score = cls->factor_score_sum / cls->weights_sum;
