...
typedef struct Vmpackst {
	Sf kap;  Sf rkap;  Sf lgi0;  Sf aa; Sf daa;  Sf fh;  Sf dfh;
		} Vmpack;

void kapcode (Sf hx, Sf hy, Sf *vmst);
...
typedef struct Sauxst {
	...
	Sf dummy;
	Datum xn;
	Sf parkfxx;	/* angle - vv*ld for thing n */
	Sf eps;
	Sf leps;  /* Log eps */
	Sf epsfac;	/* It seems that the 'increased variance' effect
...
typedef struct Basicst {     /* Basic parameter info about var in class.
	...
	Sf bhx, bhy;	/* current mean, log hy */
	Sf nhx, nhy;	/* when dad */
	Sf shx, shy;	/* when sansfac */
	Sf fhx, fhy;	/* when confac */
	Sf bhsprd, nhsprd, shsprd, fhsprd;
	Sf ld;		/* factor load */
	Sf ldsprd;
	Sf samplesize;  /* Size of sample on which estimates based */
/*	Derived quantities calced in 'adjust'	*/
/*	For Plain case:	*/
	Sf skappa, rskappa;	/* Concentration param and reciprocal */
	Sf slgi0;	/* Normalization constant */
	Sf saa;		/* deriv of slgi0 wrt kappa */
	Sf sdaa;	/* deriv of saa */
	Sf sfh;		/* one-directional Fisher for hx, hy */
	Sf sdfh;	/* deriv of sfh */
/*	For Factor case:	*/
/*	Note that fields fkappa ... fdfh match skappa ... sdfh  */
	Sf fkappa, rfkappa;	/* Concentration param and reciprocal */
	Sf flgi0;	/* Normalization constant */
	Sf faa;		/* deriv of flgi0 wrt kappa */
	Sf fdaa;	/* deriv of faa */
	Sf ffh;		/* one-directional Fisher for hx, hy */
	Sf fdfh;	/* deriv of ffh */
	Sf fmufish;	/*	Fish wrt mu  */
	Sf ldsq;	/* Square of load */
	}   Basic;
...
typedef struct Statsst  {	/* Stuff accumulated to revise Basic  */
	...
	/********************  Following fields vary according to need ***/
	Sf parkstcost, parkftcost;  /* Unweighted thing costs of xn */
	Sf oldftcost, adj;
	Sf tssin, tscos;  /* Sum of sines and cosines (no factor) */
	Sf tfsin, tfcos;  /* Sum of sines and cosines (with factor) */
	Sf ldd1, ldd2;
	Sf fwd2;	/* Total coeff of Fmu in thing costs */
	}  Stats;
	...
	Sw iv;
{
	...
	if (dad) dcvi = (Basic*) dad->basics[iv];  else dcvi = 0;
	return;
}
...
/*	Read in auxiliary info into saux, return 0 if OK else 1  */
	...
	if (i < 0) {sax->eps = sax->leps = 0.0;  return (1);  }
	i = readdf (&(sax->eps), 1);
	if (i < 0) {sax->eps = sax->leps = 0.0;  return (1);  }
	if (sax->unit) sax->eps *= (pi / 180.0);
	if (sax->eps > 0.01)
		sax->epsfac = 2.0 * sin (0.5 * sax->eps) / sax->eps;
	else
		sax->epsfac = 1.0 - sax->eps * sax->eps / 24.0;
	sax->leps = log (sax->eps);   return (0);
}
...
/*	-----------------------  costvar  --------------------------   */
/*	Accumulates thing cost into scasecost, fcasecost    */
void costvar (iv, fac)
	Sw iv, fac;
{
	Sf del, cost, tt, tsprd, cosw, sinw, r2;
	setvar (iv);
	if (saux->missing) return;
	if (cls->age == 0) { evi->parkftcost = evi->parkstcost = 0.0; return; }
...
/*	And we need -kap * cos (mu + w - xx)   */
	...
	cost -=
		(cvi->fhy * saux->xn.cosxx + cvi->fhx * saux->xn.sinxx) * cosw
...
/*	--------------------  derivvar  --------------------------  */
/*	Given the thing weight in cwt, calcs derivs of cost wrt basic
params and accumulates in paramd1, paramd2.
Factor derivs done only if fac.  */
void derivvar (iv, fac)
	Sw iv, fac;
{
	Sf tt, tsprd, r2, cosw, sinw, wtr2, wd1, dwdt, dr2dw;
	Sf coser, siner;
	setvar (iv);
	if (saux->missing) return;
/*	Do non-fac first  */
...
/*	Accumulate weighted thing cost  */
...
void adjust (iv, fac)
	Sw iv, fac;
{
	Sf adj, temp1, cnt, ldd2;
	Sf del1, del2, spcost, fpcost;
	Sf dadhx, dadhy, dhsprd;
	Sf hxd1, hyd1, hkd1, hkd2;
...
/*	Get prior constants from dad, or if root, fake them  */
	if ( ! dad)	{  /* Class is root */
		dadhx = dadhy = 0.0;  dhsprd = NullSprd;
		}
	else	{
		dadhx = dcvi->nhx;  dadhy = dcvi->nhy;
		dhsprd = dcvi->nhsprd;
		}
...
/*	Make a stab at class tcost  */
	cls->cstcost += cnt * (2.0 * hlg2pi - saux->leps + cls->mlogab) + 1.0;
...
/*	Compute parameter costs as they are  */
	...
	spcost = 2.0 * hlg2pi + log (dhsprd) +
		 0.5 * temp1 * (del1 + 2.0 * cvi->shsprd)
		 - log (cvi->shsprd) + 2.0 * lattice;

	if (! fac){fpcost = spcost + 100.0;  cvi->infac = 1;  goto facdone1; }
	del2 = (dadhx - cvi->fhx) * (dadhx - cvi->fhx) +
		(dadhy - cvi->fhy) * (dadhy - cvi->fhy);
	fpcost = 2.0 * hlg2pi + log (dhsprd) +
		 0.5 * temp1 * (del2 + 2.0 * cvi->fhsprd)
		 - log (cvi->fhsprd) + 2.0 * lattice;
...
/*	From things,
	...
	if (cvi->sfh < 0.0) kapcode (cvi->shx, cvi->shy, &(cvi->skappa));

/*	From thing cost term - kap*Sum{cos(mu-x)} :  */
	hxd1 = -evi->tssin;  hyd1 = -evi->tscos;

/*	From thing cost term N * LI0 : */
/*	Deriv wrt kappa is : */
	hkd1 = cnt * cvi->saa;
...
/*	Adjust shx, shy  */
	cvi->shsprd = 1.0 / hkd2;
	if ((cvi->shsprd * cnt) < 1.0) cvi->shsprd = 1.0 / cnt;
...
/*	Store the constant part of cost (sli0 + shsprd*Fh) in cvi->slgi0 */
	...
	if (! fac)  {	/* Set factor params from plain  */
		cvi->fhx = cvi->shx;  cvi->fhy = cvi->shy;
		cvi->flgi0 = cvi->slgi0;
		cvi->ld = cvi->ldsq = 0.0;
		cvi->fmufish = cvi->skappa * cvi->saa;
		cvi->fkappa = cvi->skappa;  cvi->fhsprd = cvi->shsprd;
		cvi->ldsprd = 1.0 / cnt;
		cvi->ffh = -1.0;
		goto facdone2;
		}
...
/*	From things,
	...
	Must get kap, Sum, LI0, Fh etc  */
	if (cvi->ffh < 0.0) kapcode (cvi->fhx, cvi->fhy, &(cvi->fkappa));

/*	From thing cost term - kap*Sum{cos(mu-x)} :  */
	hxd1 = -evi->tfsin;  hyd1 = -evi->tfcos;
...
/*	From prior cost:  */
	...
	hkd2 += 2.0 * temp1;
	evi->ldd1 += cvi->ld;  ldd2 += 1.0;

/*     Adjust fhx, fhy, ld  */
	del1 = evi->ftcost / cnt;
	adj = evi->adj * 1.1;  if (adj > 1.25) adj = 1.25;
	if (del1 > evi->oldftcost) adj = 0.5 * adj;
	if (adj < InitialAdj) adj = InitialAdj;
	evi->adj = adj;  evi->oldftcost = del1;
	cvi->fhsprd = 1.0 / hkd2;
	if ((cvi->fhsprd * cnt) < 1.0) cvi->fhsprd = 1.0 / cnt;
...
/*	Adjust as-dad params, but if no sons, set from nonfac params */
	if (cls->nson < 2)	{
		cvi->nhx = cvi->shx;  cvi->nhy = cvi->shy;
...
skipn:
	...
	printf(" B:  Mean ");
	if (saux->unit)	printf("%6.1f deg", (180.0/pi)*mu);
		else    printf("%6.3f rad", mu);
	printf("  Kappa %8.2f\n", kappa);
	return;
}
...
void ncostvar (iv, vald)
	Sw iv, vald;
{
	Basic *soncvi;
	Class *son;
	Sf pcost;
	Sf nhx, nhy, nhsprd, dadhx, dadhy, dadhsprd;
	Sf del, co0, co1, co2, meanx, meany;
	Sf tsxn, tsyn, tsvn, tssn, sbhx, sbhy, sbhsprd;
	Sw nints, nson, ison, n;
...
/*	Get prior constants from dad, or if root, fake them  */
	if (! dad)	{  /* Class is root */
		dadhx = dadhy = 0.0;  dadhsprd = NullSprd;
		}
	else	{
		dadhx = dcvi->nhx;  dadhy = dcvi->nhy;
		dadhsprd = dcvi->nhsprd;
		}
	nhx = cvi->nhx;  nhy = cvi->nhy;  nhsprd = cvi->nhsprd;
...
/*	We need to accumulate things over sons. We need:   */
	...
	for (ison = cls->ison; ison > 0;  ison = son->isib)	{
		...
		sbhx = soncvi->bhx;  sbhy = soncvi->bhy;
...
/*	First we get the V around the sons' mean, then update nhx, nhy
	...
	meanx = tsxn / nson;  meany = tsyn / nson;
...
adjloop:
/*	Update params  */
	nhx = (dadhsprd * tsxn + nhsprd * dadhx) /
			( nson * dadhsprd + nhsprd );
	nhy = (dadhsprd * tsyn + nhsprd * dadhy) /
			( nson * dadhsprd + nhsprd );
...
/*	Solve for new spread  */
	...
	n--;  if (n) goto adjloop;
	cvi->nhx = nhx;  cvi->nhy = nhy;  cvi->nhsprd = nhsprd;

adjdone:	/*	Calc cost  */
	del = (nhx-dadhx)*(nhx-dadhx) + (nhy-dadhy)*(nhy-dadhy);
	pcost = 2.0*hlg2pi + 2.0 * log (dadhsprd) +
		(0.5 * del + nhsprd/nson) / dadhsprd +
		nhsprd / dadhsprd;
	...
	Sf *vmst;
{
	...
	if (kap > 1.0e-6) vmp->rkap = 1.0 / kap;  else vmp->rkap = 1.0e6;
	skap = kap * kscale;
	ik = (int) skap;  del = skap - ik;
	if (ik >= maxik) goto asymp;

/*	Interpolate   */
	va = lgi0 [ik];  da = atab [ik] * kapstep;  ik++;
	vb = lgi0 [ik];  db = atab [ik] * kapstep;
...
/*	For large kap, logi0 asymptotes to:
	...
	*/
	...
	vmp->aa = 1.0 - 0.5 * del
		-del*del *(0.125 + del*(0.125 + del*0.201));
...
/*	Now, Fh is the square root of a/k - a^2/k^2 - a^3/k
	...
	*fsh = fh = sqrt ( aonk * (1.0 - aonk - aa * aa));
	dfh = - aonk * rkappa + 2.0 * aonk * aonk * rkappa
		+ aonk * aonk * aa
